...
REST usa recursos identificados por URLs e operacoes HTTP como `GET`, `POST`, `PUT` e `DELETE`. No projeto, REST retorna JSON e usa codigos HTTP para indicar sucesso ou erro.

Vantagens:

- Simples de entender e demonstrar.
- Facil de testar com navegador, Postman, curl ou PowerShell.
- Muito usado em APIs web.

Desvantagens:

- Pode ter overhead maior por usar HTTP/JSON.
- O cliente normalmente recebe a estrutura definida pelo servidor.
- Em cargas altas, pode formar filas dependendo do servidor utilizado.

### gRPC

gRPC usa contratos `.proto`, comunicacao binaria com Protobuf e normalmente roda sobre HTTP/2. No projeto, os servicos gRPC tem mensagens fortemente tipadas para usuarios, musicas e playlists.

Vantagens:

- Melhor desempenho geral nos testes.
- Baixa latencia.
- Contrato formal por Protobuf.
- Bom para comunicacao interna entre servicos.

Desvantagens:

- Mais trabalhoso de demonstrar manualmente do que REST.
- Exige geracao de codigo a partir do `.proto`.
- Nao e tao direto para uso em navegador sem ferramentas auxiliares.

### GraphQL

GraphQL permite que o cliente envie queries e mutations definindo quais campos deseja receber. No projeto, GraphQL implementa queries para listagem e mutations para CRUD.

Vantagens:

- Flexibilidade para o cliente escolher os campos retornados.
- Uma unica rota `/graphql` concentra consultas e alteracoes.
- Bom quando diferentes clientes precisam de formatos de resposta diferentes.

Desvantagens:

- Custo adicional de parse, validacao e resolucao das queries.
- Pode ser mais pesado sob carga alta, especialmente em Python.
- Exige cuidado na implementacao dos resolvers.

### SOAP

SOAP usa XML, envelope SOAP e contrato formal. No projeto, SOAP foi corrigido para usar XML de verdade nas requisicoes e respostas, sem JSON encapsulado.

Vantagens:

- Contrato formal e formato padronizado.
- Comum em sistemas corporativos e legados.
- Bom quando ha exigencia de integracao baseada em XML.

Desvantagens:

- Mais verboso que REST e gRPC.
- XML aumenta custo de serializacao e parsing.
- Mais trabalhoso de escrever e ler manualmente.

## Operacoes e Retornos
...
- `200`: consulta ou atualizacao realizada.
- `201`: recurso criado.
...
O gRPC usa mensagens Protobuf. Exemplo conceitual:
...
O retorno nao e JSON nem XML; e uma mensagem binaria Protobuf decodificada pelo cliente gRPC.
